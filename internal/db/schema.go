...
CREATE TABLE IF NOT EXISTS tasks (
	...
	type            TEXT NOT NULL DEFAULT 'task'
	                CHECK (type IN ('task', 'bug', 'feature', 'chore', 'spike')),
	status          TEXT NOT NULL DEFAULT 'open'
	                CHECK (status IN (
	                    'open', 'triaged', 'ready', 'in_progress',
...
CREATE TABLE IF NOT EXISTS reservations (
	...
	release_reason TEXT CHECK (release_reason IN (
	    'completed', 'expired', 'handoff', 'manual', 'recovered', NULL
	)),
	worktree_path TEXT,
	machine_id    TEXT NOT NULL DEFAULT 'default'
);

CREATE UNIQUE INDEX IF NOT EXISTS idx_reservations_active_task
ON reservations(task_id) WHERE active = 1;
...
CREATE TABLE IF NOT EXISTS sessions (
	...
	result          TEXT CHECK (result IN (
	    'completed', 'failed', 'handoff', 'interrupted', 'abandoned', NULL
	)),
	result_detail   TEXT,
	exit_code       INTEGER,
	notes           TEXT DEFAULT '',
...
CREATE TABLE IF NOT EXISTS handoffs (
	...
	error_context   TEXT,
	worktree_status TEXT CHECK (worktree_status IN (
	    'clean', 'dirty_staged', 'dirty_unstaged', 'conflicts', NULL
	)),
...
CREATE TABLE IF NOT EXISTS worktrees (
	...
	status        TEXT NOT NULL DEFAULT 'active'
	              CHECK (status IN ('active', 'cleanup_pending', 'cleaned')),
...
CREATE TABLE IF NOT EXISTS recovery_records (
	...
	reason           TEXT NOT NULL CHECK (reason IN (
	    'crash_detected', 'reservation_expired', 'manual_recovery',
	    'session_timeout', 'worktree_conflict'
	)),
	previous_status  TEXT NOT NULL,
	recovered_to     TEXT NOT NULL DEFAULT 'ready',
	worktree_state   TEXT CHECK (worktree_state IN (
	    'clean', 'dirty', 'conflicts', 'missing', NULL
	)),
...
// FTS5 must be created separately since IF NOT EXISTS doesn't work with virtual tables.
const ftsSQL = `
...
// ApplyPragmas sets required PRAGMAs on every connection per spec §3.2.
...
// OpenDB opens and configures a Solo database connection.
func OpenDB(dbPath string) (*sql.DB, error) {
	db, err := sql.Open("sqlite3", dbPath+"?_journal_mode=WAL&_foreign_keys=ON")
	if err != nil {
		return nil, err
	}
	// Force single connection to avoid SQLite locking issues with connection pool.
...
// InitSchema creates all tables, indexes, triggers, and FTS5 virtual table.
// Called by `solo init`.
func InitSchema(db *sql.DB, machineID string) error {
	// Execute main schema
	...
	var ftsExists int
	err := db.QueryRow("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='tasks_fts'").Scan(&ftsExists)
	if err != nil {
		return fmt.Errorf("checking FTS5: %w", err)
	}
	if ftsExists == 0 {
		if _, err := db.Exec(ftsSQL); err != nil {
			return fmt.Errorf("creating FTS5: %w", err)
		}
	}

	// Insert schema version
	_, err = db.Exec(`INSERT OR IGNORE INTO schema_version (version, description) VALUES (?, ?)`,
		defaultSchemaVersion, "Solo v13 initial schema")
	if err != nil {
	...
	for k, v := range defaults {
		if _, err := db.Exec(`INSERT OR IGNORE INTO config (key, value) VALUES (?, ?)`, k, v); err != nil {
			return fmt.Errorf("inserting config %q: %w", k, err)
		}
	}
...
// FindRepoRoot walks up from dir until it finds a .git entry (file or directory).
// Uses os.Stat per spec §3.1 — not exec.Command("git", ...).
func FindRepoRoot(startDir string) (string, error) {
	...
	for {
		...
		if parent == dir {
			return "", fmt.Errorf("no .git found walking up from %s", startDir)
		}
		dir = parent
	}
}
